...
/**
 * Filtered result contract handed to the results view (Contributor 3's
 * component) via `results` / `totalCount` / `filters.view` / `filters.query`:
 *
...
export function useMovieFilters(pageSize: number = CATALOG_PAGE_SIZE) {
  ...
  // Enrich with parsed year/decade; `movies` from CatalogContext is untouched.
  ...
  const filteredResults = useMemo(() => {
    ...
    // Sort a copy — never mutate the catalog or filtered arrays.
  ...
  // Reset pagination whenever the effective result set could change shape.
  useEffect(() => {
